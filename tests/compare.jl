...
function solution_ratio(sol, opt_edges::Int)
...
function compare_suites(path_a::AbstractString, path_b::AbstractString;
    ...
    for seed in shared
        ...
        opt = Int(ta.opt_edges)
        if Int(tb.opt_edges) != opt
            @warn "opt_edges disagree for graph_seed=$seed" a=Int(ta.opt_edges) b=Int(tb.opt_edges)
        end
    ...
    println()
    println("Optimal (vs oracle)  : A=$(a_optimal)  B=$(b_optimal)  both=$(both_optimal)  / $(length(shared))")
    ...
    if !isempty(a_better)
        ...
        for row in a_better
            println("  seed=$(row.graph_seed)  opt=$(row.opt_edges)  A=$(row.edges_a) (r=$(round(row.ratio_a; digits=3)))  B=$(row.edges_b) (r=$(round(row.ratio_b; digits=3)))  nU=$(row.nU) nV=$(row.nV) k=$(row.k) θ=$(row.theta)")
        end
    end
