...
Holding ants fixed at $A = 100$, we retrospectively vary the number of epochs per ant and the replicate count using data gathered in Section~4.6. We do not re-run ACO with truncated settings. For epoch budget $T\in\{1,\ldots,5\}$, we retain only replicates whose epochs-to-best (ETB) satisfies $\mathrm{ETB}\leq T$ and use their recorded final edge counts (a conservative lower bound, since intermediate quality before ETB is not stored). For replicate budget $R$, we keep only the first $R$ counted replicates per graph and take the best among them. Figures~\ref{fig:iteration-budget} and~\ref{fig:replicate-budget} share the same panel layout: percentage of graphs that beat the $\theta$-heuristic and that admit a $\theta$-feasible ACO solution; mean and median percent edge increase among $\theta$-feasible graphs (log-scaled); and a cumulative distribution function (CDF) of when the eventual best was reached. The CDF measures the probability that the best solution on a graph would have been found within $T$ epochs (Figure~\ref{fig:iteration-budget}); and the probability that the best solution on a graph would have been found with $R$ replicates in Figure~\ref{fig:replicate-budget}.
