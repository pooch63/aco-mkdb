...
Maximizing edge count alone is not enough for these applications. Without a lower bound on each side's size, the optimum is often a lopsided subgraph with many edges but little real-world significance. In the case of fraud, a single popular item purchased by thousands of unrelated users could be a massive biclique with one vertex opposite many others, yet it signals mass-market demand rather than a coordinated ring. We therefore seek only $\theta$-feasible \mbox{$k$-DB}: \mbox{$k$-DB} where both sides have at least $\theta$ vertices.
...
We introduce an Ant Colony Optimization (ACO) heuristic for the $k$-defective maximum edge biclique ($k$-MDB) problem, incorporating a neighbor-scope restriction that reduces the candidate set considered during construction. Across 37 benchmark graphs, ACO frequently finds larger and more often $\theta$-feasible solutions than the existing $\theta$-heuristic, although at substantially higher computational cost. We further show that these stronger solutions can improve downstream branch-and-pivot search, reducing its wall-clock time by up to $56\%$ on tested instances. Together, these results demonstrate ACO as a complementary approach to existing $k$-MDB heuristics, trading increased search cost for improved solution quality.

\section{Background}

\subsection{Definitions}

Define a bipartite graph as a graph $G = (U, V, E)$, where $U$ and $V$ are two disjoint sets of vertices, and $E \subseteq U \times V$ is the set of edges between $U$ and $V$. Define $n = |U| + |V|$ and $m = |E|$. Let $S = (U_S, V_S)$ denote a subset of vertices where $U_S \subseteq U$ and $V_S \subseteq V$. Let $G(S) = (U_S, V_S, E_S)$ be the induced subgraph of $S$ where $E_S = \{(u, v) \mid u \in U_S \wedge v \in V_S \wedge (u, v) \in E \}$. Define $\bar{E}(S) = \{(u, v) \mid u \in U_S \wedge v \in V_S \wedge (u, v) \not\in E \}$ be the set of non-edges in a subgraph. Let a $k$-defective biclique be a subgraph $S$ where $|\bar{E}(S)| \leq k$. Because every possible edge runs between the two sides of a $k$-DB, a subgraph with $|U_S|$ and $|V_S|$ vertices has at most $|U_S||V_S|$ edges. For a $k$-defective biclique, $|E(S)|=|U_S||V_S|-|\bar E(S)|$, with $|\bar E(S)|\le k$.
...
Alone, $F_P$ improves solution quality over plain ACO, but once neighbor-scope limit~$F_N$ is enabled it contributes nothing further (Section~4.3).
...
\textbf{Upper Bound} The upper time complexity would therefore be $\mathcal{O}(A \times T \times n^2)$ for $A$ ants advancing for $T$ epochs. In a parallelized implementation with $p$ processors, the true upper complexity bound is $\mathcal{O}(\lceil A / p \rceil \times T \times n^2)$.

\subsubsection{Practical Complexity}

In practice, however, there are two factors limiting this complexity.

\textbf{Neighbor Scope Limit} First, let $v_{last}$ be the last vertex an ant visited. $F_N$ often only visits $d_{v_{last}}$ vertices each step (Section~3.1.1). Empirically, this means the ant will visit a number of vertices equal to the graph's average degree, $d^{\mathrm{avg}} = 2m/n$, rather than the worst-case maximum vertex degree. This motivates a practical scaling model of $\mathcal{O}(\lceil A / p \rceil \times (T \times n \times d^{\mathrm{avg}} + n))$. Note that $n \times d^{\mathrm{avg}} = n \times \frac{2m}{n} = 2m$. On sparse graphs where $m \approx O(n)$, this suggests that ACO scales linearly with $n$. The practical complexity therefore reduces to $\mathcal{O}(\lceil A / p \rceil \times (T \times m + n))$. This empirical model does not scale quadratically with $n$ on sparse graphs, though dense graphs may still trigger the worst-case scenario of $n^2$. On sparse graphs where $m \ll n^2$, this suggests significantly less complexity than the theoretical bound indicates.
\
...
To check that observed runtimes follow these bounds, Figure~\ref{fig:compare-bound-time} plots $F_N$ discovery time normalized by ant count and epoch budget, $t/(A\cdot T)$, on log--log axes against each candidate bound. Fitting $\log\!\bigl(t/(A\cdot T)\bigr) = \alpha\log(\mathrm{bound}) + c$, a complexity scale proportional to runtime should give slope $\alpha \approx 1$. The naive $n^2$ fit falls well below $1$, confirming that the quadratic worst case overestimates growth, while the practical bound $T\cdot m+n$ with $T = 5$ lands closer to $1$, supporting it as the better complexity scale.
...
We ablate neighbor-scope limit~$F_N$, with prefer-smaller-side~$F_P$ as a negative control, to show that $F_N$ accounts for the practical gains. We demonstrate that ACO's improvements over the $\theta$-heuristic can meaningfully reduce branch-and-pivot runtime.
...
We evaluated ACO, $F_P$, $F_N$, and $F_{PN}$ on all benchmark graphs with 100 ants. Figure~\ref{fig:flag-ablation} summarizes the solution quality and time for all four algorithms, with variants ordered and colored by neighbor-scope $F_N$. Quality is the per-graph mean of scored edge counts over counted replicates, with $\theta$-infeasible trials contributing 0 edges but remaining in the mean. Neighbor-scope limit~$F_N$ cuts discovery time versus plain ACO; adding prefer-smaller-side~$F_P$ on top of $F_N$ ($F_{PN}$ vs.\ $F_N$) yields no further gain, and $F_P$ alone remains slower and weaker than $F_N$. Because ACO samples candidates stochastically, narrowing the option set with $F_N$ can help rather than hurt: ants concentrate probability mass on a smaller pool of locally connected vertices instead of diluting it across a large feasible set. Figure~\ref{fig:flag-feasibility} reports the corresponding graph-level $\theta$-feasibility rates (share of graphs with at least one $\theta$-feasible counted replicate).

Neighbor-scope also changes how ants spend the defect budget early in construction. %%STATISTICS:missing-at-5%% Prefer-smaller-side~$F_P$ alone does not produce the same early-budget effect; once $F_N$ is enabled, $F_P$ adds no further quality or runtime benefit over $F_N$ (Figure~\ref{fig:flag-ablation}).
